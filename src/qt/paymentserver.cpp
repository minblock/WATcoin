...
const char* BIP71_MIMETYPE_PAYMENT = "application/watcoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/watcoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/watcoin-paymentrequest";
#endif
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start watcoin: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("watcoin://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'watcoin://' is not a valid URI. Use 'watcoin:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Watcoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
