 ...
 */
static const char* BITCOIN_PID_FILENAME = "watcoind.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/watcoin-project/watcoin>";
    const std::string URL_WEBSITE = "<https://watcoin.org>";
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if watcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if watcoin is started while in a temporary directory.\n",
